...
HTTP at three scopes, so a consumer can take everything or narrow to one machine
or one server:

    /mcp                                  {label}__{server}__{tool}
    /mcp/host/{label}                     {server}__{tool}
    /mcp/host/{label}/server/{server}     {tool}

All three are backed by a single `Server` and a single session manager; the scope
...
class McpEndpoint:
    def __init__(self, registry: Registry, service: "Service") -> None:
        ...
        self._scopes: Dict[int, Tuple[Scope, Optional[str], Optional[str]]] = {}
    ...
    def _scope_for(self, ctx: Any) -> Tuple[Scope, Optional[str], Optional[str]]:
        ...
        if request is not None:
            ...
            if label and server:
                resolved = (Scope.SERVER, label, server)
            elif label:
                resolved = (Scope.HOST, label, None)
            else:
                resolved = (Scope.ALL, None, None)
            self._scopes[key] = resolved
            return resolved
        return self._scopes.get(key, (Scope.ALL, None, None))

    # -- handlers ------------------------------------------------------

    async def _on_list_tools(self, ctx: Any, params: Any) -> types.ListToolsResult:
        self._consumers.add(ctx.session)
        scope, want_label, want_server = self._scope_for(ctx)
        ...
        for connection, channel in self.registry.iter_servers():
            ...
            for tool in channel.tools:
                exposed = compose_tool_name(scope, connection.label, channel.name, tool.name)
                if exposed is None:
                    continue

                origin = f"{connection.label}/{channel.name}"
                if exposed in seen:
                ...
                tools.append(
                    types.Tool(
                        name=exposed,
                        title=f"{tool.name} · {channel.name} @ {connection.label}",
                        description=_describe(tool.description, connection.label, channel.name),
    ...
    async def _on_call_tool(self, ctx: Any, params: types.CallToolRequestParams) -> types.CallToolResult:
        scope, want_label, want_server = self._scope_for(ctx)
        found = self.registry.resolve_tool(scope, params.name, want_label, want_server)
        if found is None:
...
def _describe(description: Optional[str], label: str, server: str) -> str:
