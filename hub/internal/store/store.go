...
// PushSubscription is one browser's Web Push registration (spec.md 8.7): the
// endpoint URL the push service exposes for this subscriber, and the two keys
// needed to encrypt a message to it (RFC 8291).
type PushSubscription struct {
	...
	// Label is operator-facing ("Akos's phone") and never required.
	Label *string
}

// PushStore is the durable set of push subscriptions. There is no per-user
// scoping (spec.md has one principal, X5): every subscription is a device
// this hub's operator has opted in from, and every notification goes to all
// of them.
type PushStore interface {
	// UpsertPushSubscription creates or replaces a subscription by endpoint:
	// re-subscribing the same browser (a permission re-prompt, a service
	// worker update) updates the existing row instead of accumulating a
	// duplicate that would double every future notification. The row's id is
	// generated by the caller (uuidv7 hex, matching every other entity) and
	// only used on a fresh insert - one already registered under that
	// endpoint keeps its original id.
	...
	// URL, used to clean up after a push service answers 404/410 - the
	// browser has unsubscribed and the endpoint no longer means anything.
